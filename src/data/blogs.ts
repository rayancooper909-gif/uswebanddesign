// ─────────────────────────────────────────────────────────────────────────────
//  BLOG POSTS  —  HOW TO ADD A NEW POST
//  1. Copy one of the objects below and paste it at the TOP of the array.
//  2. Set a unique `slug` (URL-friendly, no spaces — use hyphens).
//  3. Fill in title, excerpt, date, category, author, readTime, content.
//  4. Write the blog body in `content` using Markdown.
//     Headlines: ## Heading, ### Sub-heading
//     Bold: **text**  |  Lists: - item  |  Links: [text](url)
//  5. Save the file — the blog is live instantly.
// ─────────────────────────────────────────────────────────────────────────────
...
## The Google Business Profile (GBP) — Start Here
...
Consistent NAP (Name, Address, Phone) across Yelp, BBB, Clutch, and other directories builds authority.
...
A professional site uses no more than 2 typefaces — one for headlines, one for body text. Sizes scale logically: H1 > H2 > H3 > body.

Inconsistent fonts and sizes are the #1 giveaway of a DIY site.
...
## 3. Real Photography (or Premium Illustration)

Stock photos of smiling people in suits destroy trust. Use real photos of your team, your work, and your space. If you don't have them yet, invest in high-quality illustrations or abstract visuals that feel on-brand.

## 4. Mobile-First, Always

Over 60% of web traffic is on mobile. A site that requires pinching and scrolling sideways signals that the business doesn't pay attention to detail — which is exactly the opposite of what you want to communicate.

## 5. Fast Load Time

Professionalism includes performance. A beautiful site that takes 6 seconds to load will be abandoned. Every design decision should consider its impact on load time.
...
**US Web and Design** applies all of these principles to every project. See what's possible for your business — [view our portfolio](/portfolio) or [get a free consultation](/contact).
    `.trim(),
  },
];
